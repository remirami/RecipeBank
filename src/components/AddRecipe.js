...
const AddRecipe = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!instructions.length || !instructions[0].trim()) {
      newFieldErrors.instructions = t("addRecipe.errors.minInstructions");
      isValid = false;
    }
  ...
  const addIngredientGroup = () => {
    ...
    if (
      lastGroup &&
      lastGroup.ingredients.some(
        (ingredient) => !ingredient.name || !ingredient.quantity
      )
    ) {
      setError(t("addRecipe.errors.minIngredientGroups"));
      return;
    }
  ...
  const handleSubTypeChange = (index, subType, isChecked) => {
    const newFoodType = foodType.map((item, idx) => {
      if (idx !== index) {
        return item;
      }
      ...
      if (isChecked) {
        return {
          ...item,
          contains: [...item.contains, subType],
        };
      } else {
        return {
          ...item,
          contains: item.contains.filter((type) => type !== subType),
        };
      }
    });
      ...
      <form onSubmit={handleSubmit} className={styles.formContainer}>
        ...
        <label className={styles.labelContainer}>
          ...
          >
            ...
            <option value="Pie">{t("addRecipe.options.pie")}</option>
            <option value="Bake">{t("addRecipe.options.bake")}</option>
            <option value="Cake">{t("addRecipe.options.cake")}</option>
            <option value="Pastry">{t("addRecipe.options.pastry")}</option>
