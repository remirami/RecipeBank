...
const OpenBook = ({
  ...
  console.log("Recipe dietary preferences", recipe.dietaryPreference);
  const formatTime = (minutes) => {
    if (minutes < 60) {
      return `${minutes} ${t("openBook.minutes")}`;
    } else {
      const hours = Math.floor(minutes / 60);
      const remainingMinutes = minutes % 60;
      if (remainingMinutes > 0) {
        return `${hours} ${t("openBook.hours")} ${remainingMinutes} ${t(
          "openBook.minutes"
        )}`;
      } else {
        return `${hours} ${t("openBook.hours")}`;
      }
    }
  };
        ...
        <div className={styles.leftPage}>
          ...
          <p>
            ...
            {recipe.dietaryPreference && recipe.dietaryPreference.length > 0
              ? recipe.dietaryPreference
                  .map((preference) =>
                    t(`openBook.selectionOptions.${preference}`) !==
                    `openBook.selectionOptions.${preference}`
                      ? t(`openBook.selectionOptions.${preference}`)
                      : preference
                  )
                  .join(", ")
          ...
          <p>
            ...
            {recipe.prepTime
              ...
              : t("openBook.no_prepTime")}
          </p>
          <p>
            <strong>{t("openBook.cookTime")}:</strong>{" "}
            {recipe.cookTime
              ? formatTime(recipe.cookTime)
              : t("openBook.no_cookTime")}
          </p>
          <p>
